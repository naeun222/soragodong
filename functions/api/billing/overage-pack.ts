// POST /api/billing/overage-pack — Premium 구독 cap 도달 시 1회성 추가팩 결제.
...
export async function onRequestPost(context: { request: Request; env: Env }): Promise<Response> {
  ...
  if (!OVERAGE_PACKS[pack as keyof typeof OVERAGE_PACKS]) {
    return jsonResponse({ error: 'pack 은 premium_pack 만 허용 (사용자 명시 2026-05-02: light_pack 제거)' }, 400);
  }
  const packDef = OVERAGE_PACKS[pack as keyof typeof OVERAGE_PACKS];

  // 사용자 명시 2026-05-02 ultrathink: premium_pack 은 Premium 사용자만. Light/얼리 = 차단.
