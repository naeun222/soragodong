...
## Push 정책 (사용자 명시 2026-05-01 갱신 — 사용자 증가 대비)

1. **v4-dev = 작업 브랜치 / main = 배포 브랜치**. 사용자 증가로 main 직접 수정 X. 기본 작업은 v4-dev 에서.
2. **소라고동.com / pages.dev = main 배포**. v4-dev 는 검증 후 main merge 시점에 배포 반영.
3. **push 항상 허락** — 자동 OK. 옛 batch 10 commit 정책 무효.
4. **흐름**: v4-dev checkout → 수정 → commit → push origin v4-dev → 검증 OK → main merge (fast-forward 우선) → push origin main.
5. 긴급 hotfix 만 main 직접 가능 (사용자 명시 시).
6. force push / 큰 reset 시에만 backup branch (`main-backup-YYYY-MM-DD`) 자동 생성. 일반 push 는 X.
