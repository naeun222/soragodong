...
// 사용자 명시 이력: 4,000원 → 2,000원 (2026-04-30) → 3,000원 (2026-05-01). 1USD = 1,400원 환산 → $2.14.
...
export async function deductCost(userId: string, costUsd: number): Promise<void> {
  ...
  if (costUsd <= 0) return;
  // RPC 함수로 atomic 차감 (race condition 회피). 일단 단순 update — Supabase에 stored procedure 있으면 RPC 활용 권장.
  try {
