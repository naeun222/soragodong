...
#### 다음 세션 박을 거
- [ ] api/chat.ts (Anthropic 프록시 — non-stream + streaming + budget check + usage log)
- [ ] api/usage.ts (사용자 본인 사용량 조회)
- [ ] api/account/delete.ts (회원 탈퇴 — auth.users row 삭제, withdrawAccount의 backend 짝)
- [ ] api/billing/charge.ts (포트원 충전 결제 webhook)
- [ ] api/billing/subscribe.ts (월 정액 가입)
- [ ] api/billing/refund.ts (환불 처리 — 포트원 자동 환불)
- [ ] supabase/migrations/0002_billing_usage.sql
  - soragodong_usage 테이블 (user_id, endpoint, model, tokens, cost_usd, recorded_at)
  - soragodong_billing 테이블 (user_id, credit_balance_usd, subscription_active, free_credit_granted, ...)
  - RLS: 본인 row만 SELECT (INSERT/UPDATE는 service_role만)
- [ ] 클라이언트 swap — `callAnthropic` helper에 `BACKEND_BASE` 옵션 (Phase C 활성 시 한 줄 변경)
- [ ] 결제 시점 약관 동의 모달 (legal_draft 활성)
- [ ] 첫 AI 호출 직전 국외이전 동의 모달 (cross-border)
- [ ] 결제 모달 (포트원 SDK + 충전 / 정액 가입)
- [ ] 사용량 / 잔여 토큰 UI (Settings에 "이번 달 사용량 / 충전 잔액" 표시)
- [ ] 무료 충전 토큰 ($1) 첫 가입 시 자동 부여 흐름
