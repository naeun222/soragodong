...
- `/api/billing/subscribe` / `overage-pack` / `upgrade-tier` (포트원 키 박힌 후)
...
⚠️ **0002 + 0003 아직 실행 X** — 이거 안 박히면 `/api/chat` 자체가 NO_BILLING_ROW 로 차단됨.
...
- Naeun Lab / 일반과세자 / **261-21-02592** / 722000 (주) + 525101 (부) / 자택 사업장 / soragodongapp@gmail.com
...
전자상거래법 의무. `BUSINESS_INFO` 객체에 한 번에 박힘.
...
- 새 device E2EE 복원 — cloud `_e2eeRecovery` 박힌 후 가능. 옛 cloud 데이터는 same-device 만
