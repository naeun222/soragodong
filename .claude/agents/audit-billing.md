---
name: audit-billing
description: BILLING/CHARGE/REFUND/PAYMENT 도메인 read-only audit. 잔액 += 매 갱신 / race condition / idempotency / RLS / 결제 검증 측 검토. 이전 누적 잔액 / verify-toss-receipt 중복 / atomic RPC 동작 진단 시 사용. ⭐ 가장 중요 도메인 (돈 관련).
...
7. **인증 검증** — verifyAuth + admin 측 ADMIN_USER_ID
...
12. **subscribe / upgrade-tier / overage-pack** — payments 측 imp_uid UNIQUE 우회 가능
13. **client 측 자동 backend 호출 자리** — 의도 X 호출 grep
