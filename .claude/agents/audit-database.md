---
name: audit-database
description: Database (Supabase migrations) read-only audit. tables, RLS policies, RPCs, triggers 측 검토. RLS 우회 / RPC race / migration 누락 / FK 측 진단 시 사용.
...
- `supabase/migrations/0004_subscription_tiers.sql` (light/premium tier 측 schema)
...
12. **0005 migration 검증 쿼리** — 사용자 측 실행 확인 가능 (SELECT routine_name 등)
...
## 룰
- read-only
- migration files 만 (실제 supabase Dashboard 측 = 사용자 측 검증)
- 짧은 보고
