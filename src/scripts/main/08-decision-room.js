      ...
      <div class="decision-card-home" onclick="openDecision('${d.id}')">
        <div class="decision-card-row">
          ...
          <div class="decision-card-row-text">
            <div class="decision-card-label"><img src="/character/godong-soras-call.svg" alt="" class="godong-icon godong-mood-soras-call" decoding="async"> 진행 중인 결정 · ${daysSince}일째</div>
  ...
  screen.innerHTML = `
    ...
    ${(() => {
      ...
      return stepHelpMsgs.length > 0
        ? `<button class="btn-secondary" onclick="askAIForStep('${decision.id}', '${step.id}')" style="background:linear-gradient(135deg, rgba(178,140,212,0.22), rgba(139,126,196,0.12)); border-color:rgba(178,140,212,0.45); color:#d4b8ff; font-size:14px; font-weight:600; padding:12px 16px; box-shadow:0 2px 10px rgba(139,126,196,0.20);"><img src="/character/godong-soras-call.svg" alt="" class="godong-icon godong-mood-soras-call" decoding="async"> 마법고동과 이어서 — ${stepHelpMsgs.length}개 메시지</button>`
        : `<button class="btn-secondary" onclick="askAIForStep('${decision.id}', '${step.id}')" style="background:linear-gradient(135deg, rgba(178,140,212,0.18), rgba(139,126,196,0.10)); border-color:rgba(178,140,212,0.40); color:#d4b8ff; font-size:14px; font-weight:600; padding:12px 16px;"><img src="/character/godong-soras-call.svg" alt="" class="godong-icon godong-mood-soras-call" decoding="async"> 마법고동한테 도움 받기 ✦</button>`;
    })()}
...
function _renderMagicHelpChat() {
  ...
  if (qEl) {
    qEl.innerHTML = `<div style="font-size:13px; font-weight:600; color:var(--text);"><img src="/character/godong-soras-call.svg" alt="" class="godong-icon godong-mood-soras-call" decoding="async"> ${escapeHtml(meta.title)}</div>
...
function openFinalDecisionStep(decision, step) {
  ...
  screen.innerHTML = `
    ...
    <div class="input-group">
      <div class="input-label"><img src="/character/godong-soras-call.svg" alt="" class="godong-icon godong-mood-soras-call" decoding="async"> 최종 결정</div>
