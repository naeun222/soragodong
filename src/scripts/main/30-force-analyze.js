...
async function forceAnalyze(opts) {
  ...
  try {
    ...
    // V4 fix (사용자 보고 2026-05-28 ultrathink): batch_id 즉시 영속화 (review chain 과 동일 race — 디바운스 안 하드리프레시 시 유실 → 재제출).
    saveState(true);
    try { if (typeof saveToCloudNow === 'function') await saveToCloudNow(); } catch (e) { console.warn('[force analyze batch] batch_id cloud flush:', e); }
...
async function submitChapterCleanupBatch(unprocessed) {
  ...
  if (requests.length === 0) {
    ...
    saveState(true);  // V4 fix 2026-05-28: 일일 게이트 스탬프 즉시 로컬 flush (디바운스 race 시 불필요 재실행 방지). batch 없어 cloud 즉시 PATCH 까진 불필요.
    return;
  }
  ...
  try {
    ...
    state.lastChapterCleanupAt = new Date().toISOString();
    // V4 fix (사용자 보고 2026-05-28 ultrathink): batch_id + 스탬프 즉시 영속화 (review chain 과 동일 race).
    //   review 만큼 자주 안 터지는 건 lastChapterCleanupAt 일일 게이트 + _batchSubmittedAt 12h 쿨다운 덕 — 단 race 자체는 동일하므로 같이 닫음.
    saveState(true);
...
async function submitReviewChainBatch(missingReviews) {
  ...
  try {
    ...
    // V4 fix (사용자 보고 2026-05-28 ultrathink): batch_id 즉시 영속화 — 비-force saveState() 는 로컬 400ms 디바운스 + 클라우드 500ms 디바운스.
    //   그 창 안에 강제 새로고침하면 batch_id 유실 → 다음 진입 시 가드 통과 → 매 새로고침마다 재제출 (비용 낭비 + 결과 영영 미수신).
    //   부팅 시 클라우드가 로컬을 덮어쓰므로 saveToCloudNow() 로 클라우드까지 즉시 PATCH 해야 안전.
    saveState(true);
...
async function maybeRunChapterCleanup() {
  ...
  //   직전 일요일 04:00 (디바이스 로컬) cutoff. lastForceAnalyzeAt 이 cutoff 이전이면 trigger.
  //   fire-and-forget — Opus max_tokens 2500 시간 길어서 await X. silent toast (auto:true).
  if (!isGuest && typeof forceAnalyze === 'function'
      && _shouldRunSchedule(state.lastForceAnalyzeAt, _lastWeekly4amCutoff())) {
