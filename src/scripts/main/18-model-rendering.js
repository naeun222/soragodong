...
function _renderGuestNudgeBanner() {
  ...
  return `<div class="guest-nudge-banner">
    ...
    <div class="guest-nudge-sub">지금 데이터는 이 기기에만 있어요. - 브라우저 정리되면 사라집니다. 로그인해서 안전하게 이어가기.</div>
...
// 사용자 명시 2026-05-29: 미컨펌 원자(user_verified ≠ true) 리뷰 큐 — 나 탭 맨 위. '맞아' 컨펌하면 아래 '원자 보기'(컨펌됨)로 이동.
//   시뮬 항목 제외. 최신순 정렬, 상위 5 + 나머지 접힘. renderModelItem 재사용 (맞아/아니야 버튼 내장).
function _renderNewAtomsSection() {
  const _pending = (arr) => (arr || []).filter(x => x && x.extractedFrom !== 'simulation' && x.user_verified !== true);
  const cats = [
    { cat: 'traits', badge: '🌿', arr: _pending(state.traits) },
    { cat: 'values', badge: '✨', arr: _pending(state.values) },
    { cat: 'patterns', badge: '🌀', arr: _pending(state.patterns) }
  ];
  const flat = [];
  cats.forEach(c => c.arr.forEach(it => flat.push({ it, cat: c.cat, badge: c.badge })));
  if (!flat.length) return '';
  flat.sort((a, b) => {
    const at = a.it.created_at ? new Date(a.it.created_at).getTime() : 0;
    const bt = b.it.created_at ? new Date(b.it.created_at).getTime() : 0;
    if (bt !== at) return bt - at;
    return (b.it.confidence || 0) - (a.it.confidence || 0);
  });
  const card = (w) => renderModelItem(w.it, w.cat, state[w.cat].indexOf(w.it), w.badge);
  let html = `<div class="model-section">
    <div class="model-section-title">✦ 새로 발견 — 맞아?</div>
    <div style="font-size:11px; color:var(--text-soft); margin-bottom:8px; line-height:1.6;">새로 본 것들 — '맞아' 누르면 아래로 정리돼.</div>`;
  flat.slice(0, 5).forEach(w => { html += card(w); });
  const more = flat.slice(5);
  if (more.length) {
    html += `<details class="model-collapse"><summary>+${more.length} 더</summary><div class="model-collapse-content">`;
    more.forEach(w => { html += card(w); });
    html += '</div></details>';
  }
  html += '</div>';
  return html;
...
function renderModel() {
  ...
  let html = '';
  // _renderConfirmableSection — 정체성 단일 카테고리 공용 헬퍼 (values 용)
  ...
  // V4 feat (사용자 명시 2026-05-26 ultrathink): cluster 통합 섹션 — 핵심 작동 패턴 (traits ∪ patterns).
  //   sources = [{cat, arr, badge}, ...]. 각 item 의 source category 유지 → onclick 의 category 가 정확한 state array 가리킴.
  //   sort: verified 우선 → confidence DESC → evidence_count DESC (sortItems 와 동일 규칙). idx 는 state[cat].indexOf 그대로.
  //   mini badge inline — 사용자 명시 (옵션 A 선택).
  const _renderClusterSection = (label, sources) => {
    const flat = sources.flatMap(s => (s.arr || []).map(it => ({ it, cat: s.cat, badge: s.badge })));
    if (flat.length === 0) return '';
    flat.sort((a, b) => {
      const va = a.it.user_verified === false ? 0 : 1;
      const vb = b.it.user_verified === false ? 0 : 1;
      if (va !== vb) return va - vb;
      const ca = a.it.confidence || 0;
      const cb = b.it.confidence || 0;
      if (cb !== ca) return cb - ca;
      return (b.it.evidence_count || 0) - (a.it.evidence_count || 0);
    });
    const main = flat.filter(w => (w.it.confidence || 0) >= 0.4);
    const draft = flat.filter(w => (w.it.confidence || 0) < 0.4);
    const top = main.slice(0, 1);
    const more = main.slice(1);
    let inner = `<div class="model-section"><div class="model-section-title">${label}</div>`;
    top.forEach(w => {
      const idx = state[w.cat].indexOf(w.it);
      inner += renderModelItem(w.it, w.cat, idx, w.badge);
    });
    if (more.length > 0) {
      inner += `<details class="model-collapse"><summary>+${more.length}</summary><div class="model-collapse-content">`;
      more.forEach(w => {
        const idx = state[w.cat].indexOf(w.it);
        inner += renderModelItem(w.it, w.cat, idx, w.badge);
      });
    ...
    if (draft.length > 0) {
      inner += `<details class="model-collapse model-draft"><summary>+${draft.length}</summary><div class="model-collapse-content">`;
      draft.forEach(w => {
        const idx = state[w.cat].indexOf(w.it);
        inner += renderModelItem(w.it, w.cat, idx, w.badge);
      });
  ...
  // 사용자 명시 2026-05-29: 미컨펌 원자는 맨 위 '새로 발견' 큐. '맞아' 컨펌하면 아래 '원자 보기'(컨펌됨)로 내려감.
  if (typeof _renderNewAtomsSection === 'function') { try { html += _renderNewAtomsSection(); } catch {} }

  // PR5 (사용자 명시 2026-05-29): formulation(핵심 노드) 헤드라인. 평면 항목(values/패턴/cf)은 '원자 보기'로 접어 backing.
  ...
  // 사용자 명시 2026-05-29: 컨펌된 것만 여기 (미컨펌은 위 '새로 발견' 큐). user_verified===true.
  const _confd = (arr) => (arr || []).filter(x => x && x.user_verified === true);
  html += _renderConfirmableSection('values', '네가 중시하는 것', _confd(_valuesForRender));
  html += _renderClusterSection('핵심 작동 패턴', [
    { cat: 'traits', arr: _confd(_traitsForRender), badge: '🌿' },
    { cat: 'patterns', arr: _confd(_patternsForRender), badge: '🌀' }
  ]);
  ...
  if (state.caseFormulation.version === 0 || !state.caseFormulation.version) {
    html += `<div class="model-section">
      ...
      <div style="font-size:11.5px; color:var(--text-soft); line-height:1.7; padding:10px 12px; background:rgba(255,255,255,0.02); border-radius:8px; border-left:2px solid rgba(255,255,255,0.06);">며칠 같이 지내면 너의 데이터에서 큰 그림 (자기조절 도구 · 다루어야 할 것) 을 종합해줄게 ✦</div>
    </div>`;
  } else {
    ...
    // V4 feat (사용자 명시 2026-05-26 ultrathink): cf cluster 통합 섹션 — 자기조절 도구 (strengths ∪ mechanisms).
    //   sources = [{field, arr, badge}, ...]. 각 item 의 source field 유지 → editCFItem/deleteCFItem 의 field 인자 정확.
    //   sort: _sortCFItems 와 동일 (created_at DESC). string/object hybrid 안전.
    //   mini badge inline — ✦ strengths / ⟳ mechanisms.
    const _renderClusterCfSection = (label, sources) => {
      const flat = sources.flatMap(s => (s.arr || []).map((it, originalIdx) => ({ it, field: s.field, badge: s.badge, idx: originalIdx })));
      if (flat.length === 0) return '';
      flat.sort((a, b) => {
        const aT = (a.it && typeof a.it === 'object' && a.it.created_at) ? new Date(a.it.created_at).getTime() : 0;
        const bT = (b.it && typeof b.it === 'object' && b.it.created_at) ? new Date(b.it.created_at).getTime() : 0;
      ...
      const top = flat.slice(0, 1);
      const rest = flat.slice(1);
      const cfBulletWithBadge = (w) => {
        const text = (typeof w.it === 'string') ? w.it : (w.it?.text || w.it?.name || '');
        const verified = (typeof w.it === 'object' && w.it && w.it.user_verified === true);
        const cls = 'cf-bullet' + (verified ? ' cf-verified' : '');
        const mark = verified ? '✓' : '•';
        const badgeHtml = w.badge ? `<span class="model-cluster-badge">${w.badge}</span>` : '';
        return `<div class="${cls}">
          <span class="cf-text">${mark} ${badgeHtml}${escapeHtml(text)}</span>
          <span class="cf-actions"><button onclick="editCFItem('${w.field}', ${w.idx})" title="수정">✎</button><button onclick="deleteCFItem('${w.field}', ${w.idx})" title="삭제">✕</button></span>
        </div>`;
      };
      const topHtml = top.map(cfBulletWithBadge).join('');
      const restHtml = rest.length > 0
        ? `<details class="cf-more"><summary>+${rest.length}</summary><div class="cf-list">${rest.map(cfBulletWithBadge).join('')}</div></details>`
        : '';
      return `<div class="model-item"><div class="model-item-name">${label}</div>
        <div class="model-item-desc cf-list">${topHtml}</div>
        ${restHtml}</div>`;
    };
    // 사용자 명시 2026-05-26 ultrathink: 4 클러스터 양식 — 자기조절 도구 (강점+작동 통합) + 다루어야 할 것.
    html += _renderClusterCfSection('자기조절 도구', [
      { field: 'strengths', arr: state.caseFormulation.strengths, badge: '✦' },
      { field: 'mechanisms', arr: state.caseFormulation.mechanisms, badge: '⟳' }
    ]);
...
function renderModelItem(item, category, idx, badge) {
  // V4 (사용자 보고 2026-05-13): 옛 e750781 dead code 청소에서 잘못 삭제된 confLabel + model-item-meta 복원.
  //   각 model item 아래 "확신/관찰 중/가설 N%" 칩 + "근거 N개" 표시.
  // V4 feat (사용자 명시 2026-05-26 ultrathink): cluster 통합 섹션 mini badge — 핵심 작동 패턴 (🌿 trait / 🌀 pattern) 인라인.
  const clusterBadgeHtml = badge ? `<span class="model-cluster-badge">${badge}</span>` : '';
  ...
  return `<div class="model-item${newClass}" data-cat="${category}" data-idx="${idx}">
    <div class="model-item-name">${clusterBadgeHtml}${newBadge}${escapeHtml(item.name)}</div>
