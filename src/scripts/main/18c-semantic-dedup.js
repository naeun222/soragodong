...
function _parseSemanticDedupResponse(text) {
  ...
  if (!body) {
    ...
    if (i < 0 || j <= i) {
      console.warn('[semantic_dedup] raw response — no { or } found:', s.slice(0, 300));
      return null;
    }
    body = s.slice(i, j + 1);
  }
  try {
    ...
    console.warn('[semantic_dedup] JSON parse fail:', e && e.message, '\nraw:', s.slice(0, 300), '\nbody attempt:', body.slice(0, 300));
    return null;
  }
}
...
function _validateSemanticDedupPairs(rawPairs) {
  ...
  // cap 20 — 백엔드 prompt 도 명시하지만 client 도 가드.
...
async function runSemanticDedup(opts) {
  ...
  try {
    const response = await callAnthropic({
      ...
      // 사용자 보고 2026-05-26 ultrathink: 3000 → 4000 (truncate 가드 — 페어 20 + merged description 200자 다 받으면 ~6K char 보통, buffer ↑).
      max_tokens: 4000,
