...
// V4 (사용자 명시 2026-05-25 ultrathink): batch path 의 backend 합성 broken (chat-batch.ts raw passthrough — _endpoint/_vars 무시) 확인.
//   inline 이 production diary 자동 요약 책임 path. chat-batch.ts 에 backend 합성 추가 fix 후 inline 재폐기 예정.
//   _pendingDiarySummary race marker 는 폐기 유지 — batch 가 errored 받으므로 entry 미변경, race 없음.
async function runDiaryAutoSummaryIfNeeded() {
  if (!_canAI()) return;

  // V3.13.x: 04:00 cutoff 기준 어제부터 거꾸로
  // 사용자 보고 2026-05-17 ultrathink: 앵커 = todayKey() (cutoff-aware), setDate(-i) on 그 앵커의 noon.
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (_todayDk && dateKey === _todayDk) continue;

    const entry = (state.entries || []).find(e => e.date === dateKey);
    if (!entry) continue;
    if (entry.diary) continue;
    if (entry.aiSummary) continue;
    // V4 (사용자 명시 2026-05-16 cowork): persistent retry guard — 5번 fire 사고 방지.
    if (entry._aiSummaryFailed) continue;  // empty response sentinel — 영구 skip
    ...
    // 사용자 보고 2026-05-04 (B16): testerMode 아닐 때 시드 entry 제외.
    ...
    // attempt 마킹 → saveState → cloud sync → 재진입 시 cooldown / cap 보존.
    ...
    try {
      ...
      // backend chat.ts 의 _emptyResponse sentinel — 영구 skip 마킹.
...
// diary summary prompt builder — runDiaryAutoSummaryIfNeeded 와 _buildDiaryBatchRequests 가 공유.
...
// runDiaryAutoSummaryIfNeeded 가 호출 — backend chat.ts path 거쳐 _endpoint='daily_summary' 시스템 + buildDailySummary user content 합성.
async function summarizeDayForEntry(date, messages, entry) {
  ...
  // backend empty response sentinel — chargeUsage skip 됨. 영구 skip 마킹용 객체 반환.
