async function testSeedV4Data() {
  ...
  // 사용자 명시 2026-05-01 ultrathink: 깨달음 시드 새 5 카테고리 분포 (스크랩/숙고/마법/메모) + 마법 추가.
  state.archive = [];
  const archiveSeeds = [
    { type: 'scrap', headline: '환경이 의지보다 강함',
      body: '도구 셋업 한 번이 의지 매일보다 효과 큼.', daysAgo: 4, tags: ['환경', '습관'],
      userMsg: '아무리 다짐해도 작업 시작이 안 돼. 의지가 부족한 건가?',
      assistantMsg: '의지 문제가 아닐 수도 있어. 작업 환경 한 번 셋업해두면 다음 진입 마찰이 확 줄어. 환경 = 자동 신호. 의지보다 강해.' },
    { type: 'memo', headline: '저녁 무력감 = 정상',
      body: '에너지 곡선 자연스러운 저점. 자책 X.', daysAgo: 7, tags: ['에너지', '자기이해'],
      userMemo: '에너지 곡선 자연스러운 저점. 자책 X.' },
    { type: 'reflection', headline: '학문 vs 졸업장',
      body: '학문 좋아함 + 졸업장도 필요. 둘 다 진실.', daysAgo: 10, tags: ['숙고', '진로'] },
    { type: 'magic', headline: '🌀 진로 결정 — 거리두기',
      body: '친한 친구한테 이 결정 어떻게 보일지 떠올려보니 답이 분명. 자기 거리두기 효과.', daysAgo: 12, tags: ['마법', '결정', '거리두기'] },
    { type: 'scrap', headline: '거절 = 자기 보호',
      body: '술자리 거절은 약함 X 강함. 부채감은 일시.', daysAgo: 14, tags: ['거절', '관계'],
      userMsg: '오늘 술자리 거절했는데 친구한테 미안하고 부채감이 너무 든다.',
      assistantMsg: '거절은 약함이 아니라 강함이야 — 자기 시간을 지키는 결단. 부채감은 일시적이고 며칠이면 사라져. 친구도 네 사정 이해할 거야.' },
    { type: 'memo', headline: '아침 6시 = 골든',
      body: '새벽 작업이 가장 잘 됨. 이거 활용해야.', daysAgo: 18, tags: ['시간', '리듬'],
      userMemo: '새벽 작업이 가장 잘 됨. 이거 활용해야.' },
    { type: 'magic', headline: '🔮 Future Self — 6개월',
      body: '6개월 전 예측 정확도 7/10. 우려한 부분 거의 안 일어남, 예상 못한 변수가 더 컸음.', daysAgo: 45, tags: ['마법', 'Future Self'] },
    { type: 'scrap', headline: '실패 ≠ 자기 부정',
      body: '한 번 안 통한 = 데이터, 가치 판정 X.', daysAgo: 80, tags: ['실패', '자기이해'],
      userMsg: '이번 시도도 실패. 내가 부족한 건가 싶어.',
      assistantMsg: '한 번 안 통했다 = 그 방법이 안 맞았다. 네 가치랑 별개야. 데이터 한 점 늘었어 — 다음 시도 더 정확해질 거야.' },
    { type: 'memo', headline: '카페 새벽 = 가장 강한 환경',
      body: '의지 0%여도 카페 자리 잡으면 시작됨. 환경 신호의 힘.', daysAgo: 130, tags: ['환경', '카페'],
      userMemo: '카페 새벽 = 가장 강한 환경. 의지 0%여도 자리 잡으면 시작됨.' },
    { type: 'reflection', headline: '학회 발표 후 깨달음 — 완벽보다 공유',
      body: '준비 부족 두려웠지만 발표 후 피드백이 더 큰 자산. 완벽주의 비용 ↑.', daysAgo: 190, tags: ['숙고', '완벽주의'] },
    { type: 'scrap', headline: '회복 = 일의 일부',
      body: '쉬는 날도 작업 일부. 회복 없는 폭발 = 번아웃.', daysAgo: 245, tags: ['회복', '리듬'],
      userMsg: '주말에 쉬면 일정 밀리는 게 너무 부담돼.',
      assistantMsg: '쉬는 날 ≠ 잃는 시간. 주말 회복이 평일 폭발력의 연료야. 회복도 일의 일부로 봐도 돼.' },
    { type: 'memo', headline: '몸 신호 먼저 듣기',
      body: '졸림/배고픔 무시하고 밀어붙이면 다음 날 손해 더 큼.', daysAgo: 320, tags: ['몸', '자기돌봄'],
      userMemo: '졸림/배고픔 무시 X. 다음 날 손해 더 큼.' }
  ];
  archiveSeeds.forEach((a, i) => {
    // V4-fix v3 (사용자 요청 — 깨달음 가공): revisitCount + starred 다양화
    // 자주 본 깨달음 = 살아있는 통찰 (★ 일부 표시)
    const revisitCount = (i % 4 === 0) ? Math.floor((30 - i) / 4) + 2 : Math.max(0, Math.floor(Math.random() * 3));
    const starred = (i % 7 === 1 || i % 5 === 3);  // 약 25% 즐겨찾기
    state.archive.push({
      type: a.type,
      headline: a.headline,
      body: a.body,
      // 사용자 요청 2026-04-28: scrap original = "내 대화 → 소라 대화" 페어 구조
      original: a.type === 'scrap' && a.userMsg && a.assistantMsg
        ? `[나]\n${a.userMsg}\n\n[소라]\n${a.assistantMsg}`
        : null,
      insight: a.body,
      ...(a.type === 'memo' && a.userMemo ? { userMemo: a.userMemo } : {}),
      tags: a.tags,
      date: new Date(today.getTime() - a.daysAgo * 86400000).toLocaleDateString('ko-KR'),
      source: a.type === 'memo' ? '메모' : a.type === 'reflection' ? '🌊 숙고' : a.type === 'magic' ? '🌀 마법' : '대화',
      savedAt: new Date(today.getTime() - a.daysAgo * 86400000).toISOString(),
      revisitCount,
      starred
    });
  });
  ...
  // tasks 6 (오늘의 카드 3 isToday=true + 일반 할 일 3 isToday=false)
  state.tasks = [];
  ['논문 서론 마무리', '운동 30분', '책 1챕터'].forEach((t, i) => {
    state.tasks.push({
      id: 'task_seed_' + i, title: t,
      isToday: true, status: 'pending',
      priority: i,
      createdAt: new Date(today.getTime() - 86400000).toISOString()
    });
  });
  ['책상 정리', '카드 결제', '약 사기'].forEach((t, i) => {
    state.tasks.push({
      id: 'task_seed_extra_' + i, title: t,
      isToday: false, status: 'pending',
      priority: 10 + i,
      createdAt: new Date(today.getTime() - i * 86400000).toISOString()
    });
  });
  // 사용자 요청 2026-04-28: starts (몰입 세션) 시드 — 나 탭 '시간 사용 패턴' 카드 데이터
  state.starts = [];
  // 지난 7일, hour 다양 (시간대 패턴 보이게), task 다양
  const startSeeds = [
    { task: '논문 서론 마무리', daysAgo: 0, hour: 5, durationMin: 45, outcome: 'partial' },
    { task: '논문 서론 마무리', daysAgo: 1, hour: 4, durationMin: 90, outcome: 'done' },
    { task: '운동 30분', daysAgo: 1, hour: 19, durationMin: 35, outcome: 'done' },
    { task: '논문 서론 마무리', daysAgo: 2, hour: 6, durationMin: 70, outcome: 'partial' },
    { task: '책 1챕터', daysAgo: 3, hour: 22, durationMin: 50, outcome: 'done' },
    { task: '운동 30분', daysAgo: 3, hour: 18, durationMin: 32, outcome: 'done' },
    { task: '논문 서론 마무리', daysAgo: 4, hour: 5, durationMin: 80, outcome: 'partial' },
    { task: '실험 데이터 처리', daysAgo: 5, hour: 14, durationMin: 60, outcome: 'done' },
    { task: '논문 서론 마무리', daysAgo: 6, hour: 4, durationMin: 95, outcome: 'partial' }
  ];
  startSeeds.forEach((s, i) => {
    const start = new Date(today.getTime() - s.daysAgo * 86400000);
    start.setHours(s.hour, 12, 0, 0);
    ...
    state.starts.push({
      ...
      returnedAt: end.toISOString(),
      outcome: s.outcome
    });
  });
  // memoryVault 4 (서랍장 항목 / 채팅 아이디어)
  state.memoryVault = [];
  ['주말에 카페 가서', '친구 만나서 얘기', '도서관 가서 검색', '주중 깊은 작업'].forEach((t, i) => {
    state.memoryVault.push({
      id: 'mv_seed_' + i, title: t,
      priority: 100 + i,
      createdAt: new Date(today.getTime() - i * 86400000).toISOString()
    });
  });
  // projects 2 (사용자 요청 2026-04-28: 추적 항목 2개 그래프 있는 걸로 — 풍부한 측정 데이터)
  state.projects = [
    {
      id: 'proj_seed_1', title: '체중', emoji: '⚖', kind: 'numeric',
      type: 'decrease', unit: 'kg', baseline: 65, target: 60,
      measurements: [
        { value: 65.0, date: new Date(today.getTime() - 28 * 86400000).toISOString() },
        { value: 64.5, date: new Date(today.getTime() - 24 * 86400000).toISOString() },
        { value: 64.2, date: new Date(today.getTime() - 20 * 86400000).toISOString() },
        { value: 63.8, date: new Date(today.getTime() - 16 * 86400000).toISOString() },
        { value: 63.5, date: new Date(today.getTime() - 12 * 86400000).toISOString() },
        { value: 63.4, date: new Date(today.getTime() - 9 * 86400000).toISOString() },
        { value: 63.0, date: new Date(today.getTime() - 6 * 86400000).toISOString() },
        { value: 62.8, date: new Date(today.getTime() - 3 * 86400000).toISOString() },
        { value: 62.5, date: new Date(today.getTime() - 86400000).toISOString() }
      ],
      status: 'active', showGraph: true, createdAt: new Date(today.getTime() - 28 * 86400000).toISOString(), startDate: todayStr
    },
    {
      id: 'proj_seed_2', title: '러닝 거리', emoji: '🏃', kind: 'numeric',
      type: 'increase', unit: 'km', baseline: 0, target: 30,
      measurements: [
        { value: 2,  date: new Date(today.getTime() - 21 * 86400000).toISOString() },
        { value: 4,  date: new Date(today.getTime() - 18 * 86400000).toISOString() },
        { value: 5,  date: new Date(today.getTime() - 14 * 86400000).toISOString() },
        { value: 8,  date: new Date(today.getTime() - 11 * 86400000).toISOString() },
        { value: 12, date: new Date(today.getTime() - 7 * 86400000).toISOString() },
        { value: 15, date: new Date(today.getTime() - 4 * 86400000).toISOString() },
        { value: 18, date: new Date(today.getTime() - 86400000).toISOString() }
      ],
      status: 'active', showGraph: true, createdAt: new Date(today.getTime() - 21 * 86400000).toISOString(), startDate: todayStr
    }
  ];
  ...
  const monthlyTones = [
    ...
    { flow: '겨울 — 심사숙고 시간 늘어남', pattern: '마법의 소라고동 활성화' },
  ...
  // V4-fix v3 (사용자 요청 점검): 오늘 일정 todaySchedule 시드 (V4 비전 10.6 타임테이블)
  state.todaySchedule = [
    { id: 'sch_seed_1', title: '논문 서론 작성', start: '09:00', end: '11:00', source: 'manual', taskId: 'task_seed_0', color: '#d4a76a', date: todayStr },
    { id: 'sch_seed_2', title: '점심 + 산책', start: '12:00', end: '13:30', source: 'manual', taskId: null, color: '#8fc88f', date: todayStr },
    { id: 'sch_seed_3', title: '실험 데이터 정리', start: '14:00', end: '16:30', source: 'ai', taskId: 'task_seed_1', color: '#7ec8e3', date: todayStr },
    { id: 'sch_seed_4', title: '교수님 미팅', start: '17:00', end: '18:00', source: 'gcal', taskId: null, color: '#b39ddb', date: todayStr },
    { id: 'sch_seed_5', title: '저녁 + 책', start: '19:30', end: '21:00', source: 'manual', taskId: 'task_seed_2', color: '#ff8da1', date: todayStr }
  ];
