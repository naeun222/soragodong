async function testSeedV4Data() {
  ...
  const _calKeep = '2026-04-15';
  if (Array.isArray(state.entries)) {
    state.entries = state.entries.filter(e => e && e.date === _calKeep);
  }
  if (Array.isArray(state.chatArchive)) {
    state.chatArchive = state.chatArchive.filter(a => a && a.date === _calKeep);
  }
  if (Array.isArray(state.topicCards)) {
    state.topicCards = state.topicCards.filter(c => {
      if (!c) return false;
      if (c.category === 'strategy') return true; // 양생방 DNA — 캘린더에 dot X
      const endedAt = c.chapterEndedAt || c.chapterStartedAt || c.createdAt;
      if (!endedAt) return true;
      return (typeof getDayKey === 'function' ? getDayKey(endedAt) : new Date(endedAt).toISOString().split('T')[0]) === _calKeep;
    });
  }
  if (Array.isArray(state.pearls)) {
    state.pearls = state.pearls.filter(p => {
      if (!p) return false;
      if (p.category !== '티켓' && p.category !== '책') return true; // 캘린더 dot 안 그리는 카테고리는 보존
      const dk = p.eventDate || p.finishedAt || (p.createdAt ? (typeof getDayKey === 'function' ? getDayKey(p.createdAt) : new Date(p.createdAt).toISOString().split('T')[0]) : null);
      return dk === _calKeep;
    });
  }
