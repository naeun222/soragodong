...
async function init() {
  ...
  // 사용자 보고 2026-05-13 ultrathink (perf): 옛 위치 = 인증 통과 직후 (loadFromCloud / render 전) splash hide → home 비어있을 때 사용자에게 노출 = 회전 카드 / 체크인 카드 로딩 지연 jank.
  // fix = splash hide 를 render 다 끝난 후 (아래 restoreActiveRitualOnLoad 직후) 로 이동. 인증 fail 경로 (showLoginScreen 안 hide) 는 그대로.
