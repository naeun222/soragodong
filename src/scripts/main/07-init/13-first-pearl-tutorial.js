...
async function runFirstPearlTutorialV8() {
  ...
  try {
    ...
    // 4. 코치마크 시퀀스 (V8 UI — mask off 가 default).
...
function _pearlCoachmarkIntro() {
  const body = `
    <div class="v8-coach-title">🔮 진주</div>
    <div class="v8-coach-text">
      살아있다 느꼈던 순간들 — 여기 모아.<br>
      <span class="v8-coach-text-soft">미리 시뮬용 진주 깔아뒀어 ✦ 한 바퀴 보자.</span>
    </div>
  `;
...
function _pearlCoachmarkCategories() {
  const body = `
    ...
    <div class="v8-coach-text">
      🎵 음악 / 🍴 음식 / 📍 장소 / ✨ 순간 / 👥 사람.<br>
      <span class="v8-coach-text-soft">체크인에서 같은 곡 5번 = 자동 진주가 돼 ✦</span>
    </div>
  `;
  return _v8ShowCoachmark({
