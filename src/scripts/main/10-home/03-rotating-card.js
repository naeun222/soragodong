...
4. 사건 1줄 (짧게) + 느낀 점 2-3줄 (길게). 총 2-3문장 / 120자 이하.
...
"${_userName}이 그 말 했을 때 좀 멈칫했나봐. 나만 본 것 같은데."
"오늘 ${_userName}이 mood 6이라고 했는데, 6이라기보다 4 같았어 ㅜㅜ. 근데 6이라고 적었네."
...
- 각 body **30자 이상 170자 이하** (너무 짧으면 reject. 너무 길면 잘림).
...
→ 정확히 3편 일기. 3일 전 / 2일 전 / 어제 각각 1편.
  ...
  while (attempt < 4) {
    ...
    // 사용자 보고 2026-05-11: 사건 1개 강제 — 한 entry 가 4문장 초과면 reject (사건 여러 개 의심).
    // 사용자 보고 2026-05-11 ultrathink: 5 → 4 (HANDOFF.md spec '최대 4-5문장').
    const _tooManySentences = parsed.some(e => {
      if (!e || typeof e.body !== 'string') return false;
      const _sentences = e.body.split(/[.!?…]\s|\.\s|\n\n/).filter(s => s.trim().length >= 3);
      return _sentences.length > 4;
    });
