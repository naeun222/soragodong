...
// V4 (사용자 명시 2026-05-22 ultrathink): hook generate 중단. "그만 하라고 해" — 사용자 부담 → 새 hook 생성 + push queue 등록 둘 다 disable.
//   다시 켜려면 이 _HOOK_DISABLED 상수만 false 로. preferences.hookFrequency='off' 와 다름 — 더 강한 hard gate (force/cooldown 우회 X).
const _HOOK_DISABLED = true;

async function maybeGenerateHook(opts) {
  if (_HOOK_DISABLED) return { skipped: 'disabled-by-user' };
  opts = opts || {};
...
async function _hookQueueForPush(entry) {
  if (_HOOK_DISABLED) return;  // V4 (사용자 명시 2026-05-22 ultrathink): 안전망 — generate disable 중이라 도달 X 인데 옛 path 호출 대비.
