...
function _intakeStep5Html() {
  ...
  if (_intakeState.analysisFailed) {
    ...
    // V4 (사용자 보고 2026-05-11): slice 80 → 250 + word-break — Anthropic forbidden 등 핵심 키워드 잘림 방지.
    return `
      <div class="intake-ai-msg">
        <b>🐚</b> 잠깐, 분석이 잘 안 됐어 ✦<br><br>
        <span class="small">${escapeHtml(userHint)}</span>
        ${errMsg ? `<div class="small" style="margin-top:8px; opacity:0.55; font-size:10px; word-break:break-all; max-height:120px; overflow-y:auto;">상세: ${escapeHtml(errMsg.slice(0, 250))}</div>` : ''}
      </div>
