...
function renderChatArchiveModal() {
  ...
  container.innerHTML = headerHtml + sortedArchive.map(a => {
    ...
    const headerActionsHtml = isTrash ? `
      <button class="cac-pin-btn" onclick="event.stopPropagation(); restoreChatArchive('${archId}')" title="복구" style="background:transparent; border:none; cursor:pointer; font-size:15px; padding:4px 8px;">↻</button>
      <button class="cac-pin-btn" onclick="event.stopPropagation(); purgeChatArchive('${archId}')" title="영구 삭제" style="background:transparent; border:none; cursor:pointer; font-size:15px; padding:4px 8px; color:#c44;">✕</button>
    ` : `
      <button class="cac-pin-btn" onclick="event.stopPropagation(); toggleArchivePin('${a.date}')" title="${pinTitle}" style="background:transparent; border:none; cursor:pointer; font-size:15px; padding:4px 8px; opacity:${a.pinned ? '1' : '0.55'};">${pinIcon}</button>
      <button class="cac-pin-btn" onclick="event.stopPropagation(); softDeleteChatArchive('${archId}')" title="삭제 (휴지통으로)" style="background:transparent; border:1px solid var(--border); border-radius:6px; cursor:pointer; font-size:15px; padding:4px 8px; opacity:0.85;">🗑️</button>
    `;
