...
function renderScheduleCalendarGrid(targetId, fullscreen) {
  ...
  for (const cell of visibleCells) {
    ...
    const allItems = [
      ...
      ...dt.map(t => ({
        // 사용자 명시 2026-05-27: 전체화면만 가독성 우선 — ✓ 프리픽스 제거(골드 색으로 구분). 인라인은 기존대로 ✓.
        kind: 'task',
        title: fullscreen ? `${t.title || ''}` : `✓ ${t.title || ''}`,
    ...
    html += `
      <div onclick="_schedCalDayClick('${dateKey}')" style="background:var(--surface); padding:${fullscreen ? '3px 2px 4px' : '4px 4px 5px'}; cursor:pointer; display:flex; flex-direction:column; gap:2px; box-sizing:border-box; overflow:hidden;">
        <div style="display:flex; justify-content:flex-end;">${dayLabelHtml}</div>
        ${display.map(it => `
          <div style="font-size:10px; font-weight:600; padding:${fullscreen ? '1px 4px' : '1px 5px'}; background:${it.color}; color:${it.ink}; border-radius:3px; white-space:nowrap; overflow:hidden;${fullscreen ? '' : ' text-overflow:ellipsis;'} line-height:1.4;${itemDim}" title="${escapeHtml(it.title)}">${escapeHtml(it.title)}</div>
        `).join('')}
