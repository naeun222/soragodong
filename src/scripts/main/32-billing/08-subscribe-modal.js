...
function _showRecurringConsentModal({ tier, pgLabel, isTrial }) {
  return new Promise((resolve) => {
    ...
    const trialBanner = isTrial
      ...
      : '';
    // V4 (사용자 명시 2026-05-13): 일반결제 (가계약) 모드 카피 분기 — 자동 갱신 X 명시.
    ...
    const firstPayLabel = isTrial
      ...
      : (_oneTimeMode
          ? `오늘 즉시 ${krw}원 결제 (1개월 이용권, 자동 갱신 X)`
          : `오늘 즉시 ${krw}원 결제, 이후 매월 ${nextDate.getDate()}일 자동 결제 (${nextDateStr} 부터)`);
    const rows = [
      ['상품',         _oneTimeMode ? `소라고동 ${tier.label} 1개월 이용권` : `소라고동 ${tier.label} 정기구독`],
      ['결제금액',     `<b style="color:var(--text);">${_oneTimeMode ? '' : '월 '}${krw}원</b> <span style="color:var(--text-soft); font-size:10.5px;">(부가가치세 10% 포함)</span>`],
      ['결제주기',     _oneTimeMode ? '1회 결제 (자동 갱신 X — 만료 7일 전 알림 후 직접 재구매)' : '매월 1회 자동 결제 (가입일 기준 같은 날)'],
      ['첫 결제',      firstPayLabel],
      ['결제수단',     pgLabel],
      ['해지방법',     _oneTimeMode ? '해지 X — 1개월 이용권은 만료 시 자동 종료' : `<b style="color:var(--accent);">[설정 → 구독]</b> 에서 1-click 해지 — 언제든 가능`],
...
function _showRecurringSuccessModal({ tier, plan, pgLabel, isTrial, nextBillingIso }) {
  ...
  overlay.innerHTML = `
    <div class="input-modal" style="max-width:380px; padding:24px; border-top:3px solid ${tierColor};">
...
async function openSubscribeModal() {
  ...
  const tierCard = (key, plan, recommended) => {
    ...
    // V4 (사용자 명시 2026-05-14): plan-color 강조 — Light=베이지, Plus=딥블루, Premium=황금. label 글자색 적용.
    const _tierColor = (typeof _planColorVar === 'function') ? _planColorVar(key) : 'var(--accent)';
    return `
      <div class="tier-card ${recommended ? 'tier-recommended' : ''}" style="position:relative; padding:18px 16px; background:${cardBg}; border:${cardBorder}; border-radius:14px; margin-bottom:10px; ${cardOpacity}">
        ...
        <div style="display:flex; align-items:baseline; justify-content:space-between; margin-bottom:4px;">
          <div style="font-size:18px; font-weight:700; color:${_tierColor};">${plan.emoji} ${plan.label}</div>
          ${priceHtml}
        </div>
...
async function proceedOneTimePurchase(tierKey) {
  ...
  // V4 (사용자 명시 2026-05-13): 일반결제 (1개월 일회성) 도 동의 모달 통과 — 한국 전자상거래법 §13 정합.
  //   _showRecurringConsentModal 의 isTrial=false 카피 흐름 재사용 (가계약/정기 모두 같은 카피 base 안 가계약 분기 자동 — 동의 모달 안 첫 결제 카피).
  const _consent = await _showRecurringConsentModal({ tier, pgLabel: _pgLabel(_pg), isTrial: false });
  if (!_consent) return;
  let phoneNumber = '', fullName = '';
  if (_pgInfo.needsCustomerInfo) {
