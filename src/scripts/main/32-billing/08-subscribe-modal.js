...
async function openSubscribeModal() {
  ...
  // V4 (사용자 명시 2026-05-13 ultrathink): 사전 UI 차단 매트릭스 적용 — 4 분기:
  //   (1) isCurrentPlan = 현재 구독 plan 카드 = 결제수단 변경 버튼 (Q4=b). trial 활성은 변경 X 차단만.
  //   (2) isPremiumUpgrade = 구독 중인 사용자가 Premium 카드 클릭 = 즉시 업그레이드 (Q3=b). proceedSubscribe('premium') 재활용 → backend upsert.
  //   (3) isDowngradeOrSwitch = 구독 중인 사용자가 다른 lower/equal tier 클릭 = 차단 (Q3=b — Premium 업그레이드만).
  //   (4) isFreeTrial = 미구독 + Plus + trial 미사용 = 옛 첫 달 무료 카드.
  const tierCard = (key, plan, recommended) => {
    const isCurrentPlan      = _isSubscribed && key === _curPlan;
    const isPremiumUpgrade   = _isSubscribed && !isCurrentPlan && key === 'premium';
    const isDowngradeOrSwitch = _isSubscribed && !isCurrentPlan && key !== 'premium';
    ...
    const cardOpacity = isDowngradeOrSwitch ? 'opacity:0.45;' : '';
    const recommendBadge = (recommended && !isFreeTrial && !isCurrentPlan && !isDowngradeOrSwitch)
    ...
    } else if (isDowngradeOrSwitch) {
      // 다운그레이드 / lateral switch 차단. (Q3=b — Premium 업그레이드만 허용)
      buttonHtml = `<button class="btn-secondary" disabled style="width:100%; padding:11px; opacity:0.55; cursor:not-allowed;">현재 ${TIER_PLANS_CLIENT[_curPlan]?.label || ''} 구독 중 — 변경 X</button>`;
    } else if (isPremiumUpgrade) {
      // Premium 업그레이드. 옛 구독 종료 + 새 cycle 시작 (잔여 보상 X). confirm 은 proceedSubscribe 안에서.
      buttonHtml = `<button class="btn-primary" onclick="proceedSubscribe('${key}')" style="width:100%; padding:11px;">✨ Premium 으로 업그레이드 (월 ${plan.krw.toLocaleString()}원)</button>`;
    } else {
      // 미구독 신규 가입 흐름 (기존).
...
async function proceedSubscribe(tierKey) {
  ...
  // V4 (사용자 명시 2026-05-13 ultrathink): Premium 업그레이드 흐름 — 옛 구독 종료 + 새 cycle 시작 confirm.
  //   backend `portone-register-recurring` 이 upsert 라 옛 row (subscription_plan='light' 등) 자동 덮어씌움.
  //   잔여 trial / cycle 일수 보상 X — 사용자 명시 (upgrade-tier.ts 정책과 동일).
  ...
  const _isUpgrading = _curBillingActive && _curBillingPlan && _curBillingPlan !== tierKey && tierKey === 'premium';
  if (_isUpgrading) {
    const curLabel = TIER_PLANS_CLIENT[_curBillingPlan]?.label || _curBillingPlan;
    const ok = confirm(`✨ Premium 으로 업그레이드\n\n현재 ${curLabel} 구독이 즉시 종료되고, 오늘부터 새 Premium 사이클이 시작돼.\n\n• 오늘 ${tier.krw.toLocaleString()}원 즉시 결제\n• 다음 자동 결제는 30일 후\n• 잔여 ${curLabel} 일수 보상 X (정책상)\n• 구독수단도 새 카드로 등록\n\n계속할까?`);
    if (!ok) return;
  }
