...
const TIER_PLANS_CLIENT = {
  ...
  // V4 (사용자 명시 2026-05-11 ultrathink): backend `_lib/billing.ts:ensureBillingRow` 가 신규 가입 시 자동 활성화.
  //   plan='early_light', cap $1.1 ≈ 1,400원, 30일 만료, 결제 X. 만료 후 자동 정지 (cron 갱신 X).
  //   사용자 노출 카피는 양 비공개 — '환영 토큰' / '한 달 무료 체험' 식. 절대값 (1,400원) 노출 X.
  //   ⚠ Plus 첫 달 무료 정기결제 가동 시 = 신규 사용자 60일 무료 (early_light 30일 + Plus trial 30일) 발생 — 의도 확인 필요할 수 있음.
  early_light:    { krw: 0,     cap_usd: 1.1,  cap_krw: 1400,  label: '환영 체험',                 tagline: '한 달 무료', emoji: '🐚',
    description: '신규 가입 자동 활성화 — 30일 무료 체험. 만료 후 Light/Plus/Premium 중 선택.' },
