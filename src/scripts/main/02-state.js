...
function _localSaveReplacer(key, value) {
  ...
  //   cloud(_cloudStateReplacer)도 2026-06-01(photoThumb offload)부터 동일 strip — Storage 가 SSOT. 같은 strip 로직 양쪽 동기 필수.
...
function _cloudStateReplacer(key, value) {
  ...
  // V4 fix (사용자 명시 2026-06-01 — 장기 안전 Phase 2: photoThumb offload): Storage 백업(photoStorageKeys) 있는 사진 base64 는 cloud main row 에서도 strip.
  //   배경: shellCollection 체크인 소라의 photoThumb/photos base64 (~726kB) 가 cloud row 통째 재암호화 IO 누수 (incident_2026_05_30 §B). diary entry 는 마이그 시 메모리 photos[idx]=null 이라 이미 무관.
  //   _localSaveReplacer:587-597 와 *동일 로직* — localStorage 에서 이미 검증된 strip 을 cloud 로 확장 (변경 시 양쪽 동기 필수).
  //   ⚠️ guard: photoStorageKeys 가 사진을 덮을 때만 strip. 백업 없는 사진(미션 소라 photoThumb — Storage 업로드 경로 X)은 보존 → 손실 0.
  //   복원: photoStorageKeys 가 state 에 남아(strip X) 렌더 시 hydrateDiaryPhotos 가 Storage 에서 채움 (체크인 소라 = _openCheckinShellStory 경유, 13-resume:60 직접 src 는 미션 소라만 도달). embedding 과 달리 parse 직후 in-place 복원 불필요.
  if ((key === 'photos' || key === 'photoThumb' || key === 'photo') && this) {
    const sk = this.photoStorageKeys;
    if (Array.isArray(sk) && sk.filter(Boolean).length > 0) {
      if (key === 'photos') {
