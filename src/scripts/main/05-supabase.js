  ...
  try {
    ...
    if (data.length > 0) {
      ...
      if (cloudData && cloudData._encryptedBody && cloudData._encryptedBody._e2ee) {
        ...
        // V4 fix (사용자 보고 2026-05-21 ultrathink): master key 없음 / decrypt 실패 / parse 실패 시도 평문 metaBody (preferences / version / lastSync / unlocked / modes / consentLog / _planOnboardingShown 등) 만 적용.
        //   root cause: 옛 path = state = DEFAULT 만 → cloud 의 평문 metaBody 무시 → 시크릿 창 / 새 device 진입 시 _planOnboardingShown / consentLog / hookFrequency 등 모두 빈 채 → onboarding/consent 모달 부메랑.
        //   safe: metaBody 는 이미 평문 cloud 저장 (saveToCloudNow line 330-348) — 통신/server 노출 동일. sensitive body 만 _encryptedBody 안.
        //   saveToCloud 는 _e2eePendingRecovery 가드로 차단됨 → 부분 state 가 sensitive cloud 덮어쓸 risk X.
        //   사용자 비밀번호 입력 → decrypt 성공 → 07-e2ee-recovery-modal.js:87-88 가 state = { ...DEFAULT, ...metaPart, ...decryptedBody } 으로 정상 복원.
        const _metaPart = (() => {
          if (!cloudData) return {};
          const { _encryptedBody, ...rest } = cloudData;
          return rest;
        })();
        if (!_e2eeMasterKey) {
