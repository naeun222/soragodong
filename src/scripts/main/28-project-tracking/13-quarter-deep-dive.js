...
function renderArchiveReviews() {
  ...
  container.innerHTML = chipHtml + annualCardHtml + filtered.map((r, idx) => {
    ...
    // V4 (사용자 명시 2026-05-29): preference 시스템 폐기 — 모음 진입 = 항상 inline classic.
    //   Story 보고 싶으면 inline 의 "Story로 보기" hint click.
  ...
  if (detail.style.display === 'none' || !detail.innerHTML) {
    ...
    detail.innerHTML = `
      <div class="wid-inner" onclick="event.stopPropagation();">
        ...
        ${(typeof _switchToStoryFromInline === 'function') ? `<button class="review-layout-hint" type="button" onclick="event.stopPropagation(); _switchToStoryFromInline('${reviewId}')">Story로 보기</button>` : ''}
      </div>
    `;
